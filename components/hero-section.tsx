...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-[hsl(var(--hero-dark))] to-[hsl(var(--hero-light))] overflow-hidden">
            ...
            {[
              {
                ...
                detail: "Weltweit im Einsatz",
              },
              {
                icon: "🌍",
                value: "50+",
                label: "Länder",
                detail: "Auf allen Kontinenten",
              },
              {
                icon: "🏆",
                value: "125+",
      ...
      {showHelp && (
        /* biome-ignore lint/a11y/noStaticElementInteractions: decorative backdrop; closes via Escape/button too */
        /* biome-ignore lint/a11y/noNoninteractiveElementInteractions: decorative backdrop; closes via Escape/button too */
        <div
        ...
        >
          {/* biome-ignore lint/a11y/noStaticElementInteractions: stopPropagation panel, no interactive intent */}
          {/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: stopPropagation panel, no interactive intent */}
          <div
            className="bg-white rounded-xl p-8 max-w-md shadow-2xl"
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => {
              // Escape closes the overlay even when focus is inside the panel;
              // everything else must not bubble to the backdrop handler.
              if (e.key === "Escape") {
                setShowHelp(false);
              } else {
                e.stopPropagation();
